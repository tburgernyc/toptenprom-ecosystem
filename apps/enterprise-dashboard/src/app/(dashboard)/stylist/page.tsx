...
async function StylistStats({
  ...
  const myStats = allStylists.find((s) => s.stylistId === stylistUserId);
...
function StatTile({
  ...
  return (
    <div className="rounded-[var(--radius-lg)] bg-[var(--color-surface-overlay)] p-4">
      <p className="text-xs text-[var(--color-foreground-muted)]">{label}</p>
      <p className="mt-1 text-2xl font-bold tabular-nums" style={{ color }}>
        {value}
      </p>
      <p className="text-xs text-[var(--color-foreground-subtle)]">{subLabel}</p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-2">
      {["9:00 AM", "11:00 AM", "2:00 PM", "4:00 PM"].map((time) => (
        <div
          key={time}
          className="flex items-center gap-3 rounded-[var(--radius-md)] bg-[var(--color-surface-overlay)] px-3 py-2.5 text-sm"
        >
          <span className="w-16 flex-shrink-0 font-mono text-xs text-[var(--color-foreground-muted)]">
            {time}
          </span>
          <span className="flex-1 text-[var(--color-foreground-subtle)] text-xs">
            Available
          </span>
        </div>
      ))}
    ...
    <div className="grid grid-cols-7 gap-2">
      {days.map((day) => (
        <div key={day} className="text-center">
          <p className="mb-1 text-xs font-medium text-[var(--color-foreground-muted)]">
            {day}
          </p>
          <div className="h-16 rounded-[var(--radius-md)] bg-[var(--color-surface-overlay)]" />
        </div>
      ))}
    </div>
  );
