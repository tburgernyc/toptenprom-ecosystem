...
async function ManagerKPIs({
  ...
  const kpis = [
    { label: "Total Bookings", value: summary.totalBookingsThisPeriod, color: "var(--color-primary)" },
  ...
  return (
    <div className="space-y-2">
      {slots.map((slot) => (
        <div
          key={slot.time}
          className="flex items-center gap-4 rounded-[var(--radius-md)] bg-[var(--color-bg-glass)] px-3 py-2 text-xs"
        >
          <span className="w-16 flex-shrink-0 font-mono text-[var(--color-text-muted)]">
            {slot.time}
          </span>
